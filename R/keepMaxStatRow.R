keepMaxStatRow <- function(matrix, keys, keepNArows=TRUE,
  ...
  haltifnot(is.function(stat),
            msg="'stat' must be either a function for some certain statistic, e.g. sd, or the name of such a function\n")
  ...
  max.rows <- sapply(1:nlevels(matrix.indexed.fac),
                     function(x) rows.by.index[[x]][ which.max(stat.by.index[[x]])])
  if(is.list(max.rows)) max.rows <- unlist(max.rows) ## in case some features got lost due to NA statistic
