...
annotateAnyIDs <- function(ids, orthologue = FALSE, multiOrth = FALSE) {
  ...
  ann <- querydbTmpTbl(comm, "m.ANY_ID", ids, "bin",
                       ribiosAnnotation:::ORACLE.BIN.USER, 
                       ribiosAnnotation:::ORACLE.BIN.PWD)
  ...
  if (!orthologue) {
    ...
    ort <- ribiosAnnotation:::annotateHumanOrthologsNoOrigTax(ann$OrigGeneID, 
                                           multiOrth = multiOrth)
    if (multiOrth) {
  ...
  rownames(res) <- ribiosAnnotation:::id2rownames(res[, cn])
  return(res)
}
